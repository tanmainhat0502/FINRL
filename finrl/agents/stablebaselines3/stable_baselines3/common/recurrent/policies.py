...
class RecurrentActorCriticPolicy(ActorCriticPolicy):
    def __init__(
        ...
        # Khởi tạo xLSTM cho critic nếu enabled
        if self.enable_critic_lstm:
        ...
        # Điều chỉnh action_net để khớp với đầu ra của mlp_extractor (64)
        self.action_net = nn.Linear(64, action_dim)  # Thay đổi in_features thành 64
    ...
    def forward(self, obs, lstm_states: Tuple[th.Tensor, ...], episode_starts: th.Tensor, deterministic: bool = False):
        ...
        values = self.value_net(latent_vf)
        # Trả về dưới dạng RNNStates
        return actions, values, log_prob.sum(dim=1), RNNStates(pi=(lstm_states_pi[0], lstm_states_pi[1]), vf=(lstm_states_vf[0], lstm_states_vf[1]))
