...
# from finrl.agents.stablebaselines3.stable_baselines3.common.utils import FloatSchedule
...
class RecurrentPPO(OnPolicyAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        # Thay lstm_actor bằng xlstm_actor vì sử dụng xLSTMBlockStack
        self.xlstm = self.policy.xlstm_actor

        if not isinstance(self.policy, RecurrentActorCriticPolicy):
            raise ValueError("Policy must subclass RecurrentActorCriticPolicy")

        # Sử dụng dummy hidden state shape vì xLSTM không cần trạng thái ẩn
        single_hidden_state_shape = (1, self.n_envs, self.policy.lstm_hidden_state_shape[2])  # Dummy shape
        self._last_lstm_states = RNNStates(
            (
                th.zeros(single_hidden_state_shape, device=self.device),
                th.zeros(single_hidden_state_shape, device=self.device),
        ...
        # Sử dụng dummy shape cho buffer vì xLSTM không cần hidden states
        hidden_state_buffer_shape = (self.n_steps, 1, self.n_envs, self.policy.lstm_hidden_state_shape[2])
